...
func findBinary() (string, error) {
	...
	if _, err := os.Stat(binPath); err != nil {
		if os.IsNotExist(err) {
			return "", driver.NewRuntimeError(fmt.Sprintf("rtl: binary not found at path: %s", binPath))
		}
		return "", driver.NewRuntimeError(fmt.Sprintf("rtl: failed to stat binary path: %s", err.Error()))
	}
